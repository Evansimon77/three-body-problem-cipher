...
// ---- key agreement (Phase 8.5): classical DH + post-quantum hybrid (port of keyexchange.py + pq_keyexchange.py) ----
//
// How two parties who share NO secret agree on one over an open wire — how the chaos cipher's master key
// is born without ever mailing it. Unlike the chaos keystream, NONE of this is hand-rolled:
//   * the CLASSICAL half is textbook finite-field Diffie-Hellman over RFC 3526 MODP Group 14 (the same
//     2048-bit prime and pure `pow` as keyexchange.py — it reuses the existing `ruint` big-int);
//   * the POST-QUANTUM half is ML-KEM-768 (FIPS 203) from RustCrypto's reviewed `ml-kem` crate, proven
//     (see the parity tests) to produce identical keys to Python's OpenSSL 3.5 backend.
// The only thing this code adds is the plumbing + the hybrid COMBINER that mixes BOTH shared secrets, so
// the session key survives a future quantum break of DH *or* a classical break of the newer ML-KEM.
//
// Determinism — the SAME discipline as twolock's explicit nonces: the caller owns ALL randomness. DH
// takes the secret exponent explicitly; ML-KEM takes the 64-byte keygen seed and the 32-byte encapsulation
// message explicitly (the crate is built WITHOUT the getrandom feature, so no OS entropy leaks in). Every
// output here is therefore a pure function of its inputs and the KAT can pin it; in real use those inputs
// come from a CSPRNG. UNAUTHENTICATED, like the Python shells: this stops a passive recorder (classical
// AND post-quantum), not an active man-in-the-middle — that is the separate authenticated handshake (8.6).

use ml_kem::kem::{Decapsulate, FromSeed, KeyExport, TryKeyInit};
use ml_kem::{MlKem768, Seed};

/// RFC 3526 MODP Group 14, the 2048-bit safe prime — byte-identical to keyexchange.py's P.
...
const DH_G: u64 = 2;
/// Fixed-width DH element encoding: 2048 bits = 256 bytes (mirrors keyexchange.py's to_bytes(256)).
...
/// ML-KEM-768 fixed sizes (FIPS 203): 64-byte keygen seed (d‖z), 1184-byte encapsulation key,
/// 1088-byte ciphertext, 32-byte encapsulation message and shared secret.
...
/// Parse a big-endian DH value. None if it does not fit the 2048-bit group (length guard avoids a panic).
...
/// A valid own/peer exponent or group element lies in [2, p-2] (mirror DHParty's range check). That range
/// already excludes the small-subgroup values 0, 1, p-1 that would force the secret into a tiny set.
fn dh_in_range(x: DhUint, p: DhUint) -> bool {
    let two = DhUint::from(2u64);
    x >= two && x <= p - two
}

/// g^private mod p — the public value safe to send on the wire. None if the private is out of range.
...
/// The validated raw DH group element g^(ab) mod p as 256 fixed-width bytes — NO KDF (mirror
/// raw_shared_secret). Both our own private and the peer value are range-checked; None on any bad input.
...
/// SHA-512 KDF of the raw shared element down to a 32-byte master key (mirror shared_key). We never use
/// the raw group element directly — it has algebraic bias; the domain-separated hash makes it uniform.
...
/// Deterministically regenerate the ML-KEM-768 keypair from its 64-byte seed (d‖z). Proven to match
/// OpenSSL's `from_seed_bytes` byte-for-byte, so the seed is the portable, KAT-pinnable private key.
...
/// The 1184-byte encapsulation (public) key for a given seed — what the initiator publishes.
...
/// Deterministic ML-KEM-768 encapsulation against a peer's public key, with the 32-byte message pinned by
/// the caller. Returns (ciphertext, shared_secret). None if the public key or message length is invalid.
...
/// ML-KEM-768 decapsulation: regenerate the secret key from the seed, recover the shared secret from the
/// ciphertext. Deterministic (and identical to OpenSSL). None on a wrong-length seed or ciphertext.
...
/// Left-pad a big-endian DH value to the fixed 256-byte transcript width (mirror _enc_dh).
...
/// The hybrid combiner (NIST SP 800-56C concatenation style, mirror _combine + _transcript): hash BOTH
/// shared secrets together with the full transcript, each field length-prefixed. The transcript binds
/// every public value, so any tampered handshake message yields a different key (fails closed). The final
/// key is unknown unless you hold BOTH secrets — it survives the loss of either DH or ML-KEM.
...
/// Responder side of the hybrid handshake (mirror HybridResponder.respond). Given the initiator's DH
/// public and ML-KEM public key, our own DH private, and a pinned encapsulation message, returns
/// (our DH public, the KEM ciphertext to send back, the agreed session key). None on any invalid input.
...
/// Initiator side of the hybrid handshake (mirror HybridInitiator.shared_key). With our own DH private and
/// ML-KEM seed, given the responder's DH public and KEM ciphertext, derive the same session key. None on
/// any invalid input. Both sides build the identical transcript (initiator DH, responder DH, initiator
/// KEM pk, ciphertext), so the keys agree.
pub fn hybrid_initiator_key(
    ...
    Some(hybrid_combine(&classical, &pq, info, &dh_a, dh_peer_b, &kem_pk_a, kem_ct))
}
...
mod tests {
    ...
    fn dh_rejects_degenerate_and_out_of_range() {
        ...
        // peer values in a tiny subgroup (0, 1, p-1) and the modulus itself are all rejected
        ...
        // a private of 0/1 is itself out of range
        assert!(dh_public(&[1u8]).is_none());
        // an over-wide value (257 bytes) is rejected, not a panic
        assert!(dh_public(&[0xffu8; DH_BYTES + 1]).is_none());
    }

    #[test]
    fn mlkem_encaps_decaps_roundtrip() {
        // Deterministic keygen + encaps + decaps recover the same secret; a different seed does not.
        ...
        // determinism: same seed + same message -> identical ciphertext
        let (ct2, _) = mlkem_encapsulate(&ek, &m).unwrap();
        assert_eq!(ct, ct2);
        // a different decapsulation seed yields a different (implicit-reject) secret, never the real one
    ...
    fn hybrid_handshake_agrees() {
        // Full two-message hybrid: initiator (DH a + ML-KEM seed) and responder (DH b) agree one key.
        let dh_a = [0x11u8; 32];
        let dh_b = [0x22u8; 32];
        ...
        // responder produces (its DH public, the KEM ciphertext, its key)
        let (dh_b_pub, ct, key_b) = hybrid_respond(&dh_b, &dh_a_pub, &kem_pk_a, &m, info).unwrap();
        // initiator, from the responder's public values, derives the same key
        ...
        // tamper: a flipped ciphertext byte -> the transcript binding breaks the agreement
