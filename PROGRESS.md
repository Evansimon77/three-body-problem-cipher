...
- [~] **Phase 4 — Rust core (the speed blocker):** IN PROGRESS — Stage A done 2026-06-29.
      ...
      - [ ] **§4 precomputed reciprocal (Stage B)** — replace the Stage-A big-int division (`div_step`,
            currently `ruint` U256, NOT yet constant-time) with a hand-rolled constant-time reciprocal.
            PLAN: Möller-Granlund "improved division by invariant integers" — normalize the divisor,
            precompute a 128-bit reciprocal ONCE at key setup, then per-step 2-by-1 division = multiply-high
            + correction, no hardware divide on the secret. Closes timing-leak #2 + drops `ruint` from the
            hot loop (more speed). Verify against `ruint` as a reference oracle over millions of random
            (num,den) pairs in a Rust test, then re-check the KAT stays bit-identical.
      - [ ] parallel maps + CTR blocks. [ ] port multimap/ratchet (needs SHA-512/HMAC in Rust) +
            extend KAT parity to those layers. [ ] differential fuzz Rust==KAT; benchmark vs AES/ChaCha.
